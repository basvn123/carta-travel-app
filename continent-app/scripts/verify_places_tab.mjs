// Headless verify for the Destinations tab v2 (.places-tab): six category
// tabs (Trips, Trails, Beaches, Lakes, Mountains, Cycling), photo cards
// everywhere, country flag cards as the index, sort chips, and the trip
// sheet with the route drawn on a real map. The priced catalogue itself
// (the old General category) lives on Explore now; see verify_explore.mjs.
//
...
// Six: Trips, Trails, Beaches, Lakes, Mountains, Cycling. Asserted by NAME
// rather than by count, so the next category to arrive does not fail this
// check for existing, and General's absence IS asserted: it left the rail
// when Explore took over the priced catalogue.
...
check('every category tab renders',
  ...
  catNames.join(', '));
check('General is off the rail', !catNames.some((c) => /general/.test(c)), catNames.join(', '));
check('Trips starts active', /trips/i.test(await page.locator('.places-cat.on').innerText().catch(() => '')));

// ── Trails: country flag cards first ──
...
await page.screenshot({ path: 'shots/places-trails-countries.png' });

// ── Trails: pick a country -> photo cards + sort chips ──
await page.locator('.places-country:visible').selectOption('SI');
...
const dcards = await page.locator('.places-tcard').count();
check('trail photo cards render', dcards > 5, `${dcards} cards`);
// The card is a bordered object with the photograph on top and the facts
// in a body strip: the old overlay stacked kind, loop, highlights, grade and
// rating over the picture, and the same slot held different things on
// different cards.
const dImg = await page.locator('.places-tcard .places-card-img').first().getAttribute('src').catch(() => '');
check('trail cards carry a real image', /upload\.wikimedia|^http/.test(dImg || ''), (dImg || '').slice(0, 60));
check('trail cards keep their facts off the photograph',
  await page.locator('.places-tcard .tcard-body .places-card-facts').count() > 5);
const dRating = await page.locator('.places-tcard .score-chip').first().innerText().catch(() => '');
check('trail cards carry a rating chip', /^\d/.test(dRating.trim()), dRating);
...
// The one open filter row is labelled; the rest stand behind Filters.
const facetLabels = await page.locator('.places-facets .places-facet-label').allInnerTexts();
check('the open filter row is labelled', facetLabels.length === 1 && /length/i.test(facetLabels[0]),
  facetLabels.join(' | '));
await page.screenshot({ path: 'shots/places-trails-cards.png' });

// Length sort flips the order to shortest-first.
await page.locator('.places-sort', { hasText: /length/i }).click();
await page.waitForTimeout(800);
const kmFirst = (await page.locator('.places-tcard .places-card-facts').allInnerTexts())
  .slice(0, 5).map((x) => parseFloat(x));
check('length sort resorts the cards', kmFirst.every((v, i, a) => i === 0 || a[i - 1] <= v), kmFirst.join(', '));
...
const tKind = await page.locator('.places-tcard .tcard-chip.is-kind').first().innerText().catch(() => '');
...
// The layer's own chips: the way up stands in the open row, labelled; the
// whole set (ways up, how hard, the kinds) is behind the Filters door.
const openLabels = await page.locator('.places-facets .places-facet-label').allInnerTexts();
check('the open mountain row is labelled', openLabels.length === 1, openLabels.join(' | '));
await page.locator('.places-filter-btn:visible').first().click();
await page.waitForTimeout(700);
const mtnChips = await page.locator('.fsheet-places .fchip').allInnerTexts();
...
for (const [blankCat, blankRe] of [['trips', /^trips$/i],
  ...
  ['mountains', /mountains/i], ['cycling', /cycling/i]]) {
...
await desk.locator('.side-cat', { hasText: /trails/i }).click();
await desk.waitForTimeout(1200);
...
check('trips index carries no lede line', await desk.locator('.jsec-lede').count() === 0);
